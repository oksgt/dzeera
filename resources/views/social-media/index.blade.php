...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{ route('social-media.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
    ...
    <div class="card card-body border-0 shadow table-wrapper ">
        <div class="card-header border-0 d-flex flex-column flex-lg-row align-items-center justify-content-start">
            <div class="mb-0">
                <form action="{{ route('social-media.index') }}" method="GET">
            ...
            <table class="table table-hover table-auto">
                <thead>
                    <tr>
                        ...
                        <th class="border-gray-200">
                            <a href="{{ route('social-media.index', ['sort' => 'url', 'dir' => ($column == 'url' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                URL {!! ($column == 'url') ? '<i class="fas fa-sort-' . (($direction == 'asc') ? 'up' : 'down') . '"></i>' : '' !!}
                            </a>
                        </th>
                        <th class="border-gray-200">
                            <a href="{{ route('social-media.index', ['sort' => 'icon', 'dir' => ($column == 'icon' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                Icon {!! ($column == 'icon') ? '<i class="fas fa-sort-' . (($direction == 'asc') ? 'up' : 'down') . '"></i>' : '' !!}
                            </a>
                        </th>
                        <th class="border-gray-200">
                            <a href="{{ route('social-media.index', ['sort' => 'is_active', 'dir' => ($column == 'is_active' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                Status {!! ($column == 'is_active') ? '<i class="fas fa-sort-' . (($direction == 'asc') ? 'up' : 'down') . '"></i>' : '' !!}
                            </a>
                        </th>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a type="button" class="btn btn-sm btn-secondary d-inline-flex align-items-center"
                                    href="{{ route('social-media.delete', ['socialMedia' => $row]) }}">
                                    Delete
                                </a>
                            </td>
                        </tr>
